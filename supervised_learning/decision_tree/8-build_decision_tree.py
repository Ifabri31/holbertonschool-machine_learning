...
class Decision_Tree():
    ...
    def possible_thresholds(self, node, feature):
        """
        Computes the possible thresholds for a given feature
        in the current node's sub_population.
        """
    ...
    def Gini_split_criterion_one_feature(self, node, feature):
        ...
        gini_impurities = np.zeros(len(thresholds))
        for i, threshold in enumerate(thresholds):
            left_mask = feature_values > threshold
            right_mask = ~left_mask
            left_counts = np.bincount(target_values[left_mask])
            right_counts = np.bincount(target_values[right_mask])
            left_total = max(1, left_counts.sum())
            right_total = max(1, right_counts.sum())
            ...
            gini_impurities[i] = (left_weight * gini_left +
                                  right_weight * gini_right)
        if len(thresholds) > 0:
            min_idx = np.argmin(gini_impurities)
            return thresholds[min_idx], gini_impurities[min_idx]
        else:
            return None, np.inf

    def Gini_split_criterion(self, node):
        """
        Computes the Gini impurity for each feature and returns the best one.
        """
        X = np.array([
